...
from wcode.net.CNN.modules.baseblocks import ConvBlock
...
class DownBlock(nn.Module):
    def __init__(
        ...
        self.downpool = pool_layer(down_scale_factor)
    ...
    def __init__(self, params):
        ...
        for i in range(len(self.num_conv_per_stage) - 1):
            self.Encoder_layers.append(
                DownBlock(
                    in_channels=self.in_channels if i == 0 else features[i - 1],
...
if __name__ == "__main__":
    ...
    data = open_yaml("./wcode/net/examples/UNet_test.yaml")
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    print("-----UNet2d-----")
    unet2d = UNet(data["Network2d"]).to(device)
    begin = time.time()
    with torch.no_grad():
        inputs = torch.rand((16, 1, 256, 256)).to(device)
        outputs = unet2d(inputs)
    ...
    total = sum(p.numel() for p in unet2d.parameters())
    print("Total params: %.3fM" % (total / 1e6))

    print("-----UNet3d-----")
    unet3d = UNet(data["Network3d"]).to(device)
    begin = time.time()
    with torch.no_grad():
    ...
    if isinstance(outputs["pred"], (list, tuple)):
        ...
        print(outputs["pred"].shape)
    total = sum(p.numel() for p in unet3d.parameters())
    print("Total params: %.3fM" % (total / 1e6))
