...
def load_stunet_pretrained_weights(network, fname, verbose=False):
    ...
    if num_inputs > 1:
        pretrained_conv1_weight = pretrained_dict[
            "conv_blocks_context.0.0.conv1.weight"
        ]
        pretrained_conv3_weight = pretrained_dict[
            "conv_blocks_context.0.0.conv3.weight"
        ]
        pretrained_dict["conv_blocks_context.0.0.conv1.weight"] = (
            pretrained_conv1_weight.repeat(1, num_inputs, 1, 1, 1)
        )
        pretrained_dict["conv_blocks_context.0.0.conv3.weight"] = (
            pretrained_conv3_weight.repeat(1, num_inputs, 1, 1, 1)
        )
