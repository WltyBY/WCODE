...
class Sam(nn.Module):
    ...
    def __init__(
        ...
        self.whether_normalize = whether_normalize
    ...
    def forward(self, batched_input, multimask_output, image_size=None):
        if isinstance(batched_input, list):
            outputs = self.forward_predict(batched_input, multimask_output)
        else:
            outputs = self.forward_train(batched_input, multimask_output, image_size)
        return outputs
    
    def forward_train(self, batched_input: torch.Tensor, multimask_output: bool, image_size: int):
        # batched_input in [b, c, h, w]
    ...
    def forward_predict(
        ...
        input_images = torch.stack(
            [self.preprocess(x["image"].to(self.device)) for x in batched_input], dim=0
        )
        image_embeddings = self.image_encoder(input_images)
    ...
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # the size of x can be [b, c, h, w] or [c, h, w]
        # the size of self.pixel_mean and self.pixel_std is [3, 1, 1]
        # Normalize colors
        if self.whether_normalize:
