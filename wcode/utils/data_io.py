...
file_endings_for_sitk = {".nii.gz", ".mhd", ".nii", ".nrrd"}
file_endings_for_2d_img = {".tif", ".jpg", ".png"}


def get_all_img_and_label_path(
    dataset_name: str, files_ending: str, channel_names: Dict[str, str]
) -> Dict:
    """
    Get all image and label paths from the dataset folder.

    Args:
        dataset_name: Name of the dataset
        files_ending: File ending for images and labels (e.g., '.nii.gz')
        channel_names: Dictionary mapping channel names to their file endings
            e.g., {"0": "CT", "1": "PET-CT"}

    Returns:
        output_dict: Dict
        save in folders "images" and "labels"
        {
            'patient_0001': {'image': ['Modality0_path', ...], 'label': ['Label_path']},
            'patient_0002': {'image': ['Modality0_path', ...], 'label': ['Label_path']},
            'patient_0003': {'image': ['Modality0_path', ...], 'label': ['Label_path']},
            ...
        }
        save in folders "imagesTr", ("imagesTs"), "imagesVal", and "labelsTr", ("labelsTs"), "labelsVal",
        {
            "train": {
                'patient_0001': {'image': ['Modality0_path', ...], 'label': ['Label_path']},
                ...
            ...
            "val": {
                'patient_0005': {'image': ['Modality0_path', ...], 'label': ['Label_path']},
                ...
            },
            ("test"): {
                'patient_0010': {'image': ['Modality0_path', ...], ('label': ['Label_path'])},
                ...
            }
        }
        whether_need_to_split: Bool
    """
    dataset_folder_path = os.path.join("./Dataset", dataset_name)
    folder_lst = [
        i
        for i in os.listdir(dataset_folder_path)
        if os.path.isdir(os.path.join(dataset_folder_path, i))
    ]
    files_ending_lst = [
    ...
    if {"images", "labels"}.issubset(folder_lst):
        ...
        custom_name = train_images_name_lst[0].split("_")[0]
        output_dict = dict()
        for dataset_split in names_dict.keys():
            output_dict[dataset_split] = {}
            for img in names_dict[dataset_split]["images"][1]:
                case_id = img.split("_")[1]
                if case_id not in output_dict[dataset_split].keys():
                    output_dict[dataset_split][case_id] = {}
                    output_dict[dataset_split][case_id]["image"] = []
                label = "{}_{}{}".format(custom_name, case_id, files_ending)

                output_dict[dataset_split][case_id]["image"].append(
                    os.path.join(names_dict[dataset_split]["images"][0], img)
                )

                if not have_test_labels and dataset_split == "test":
                    pass
                else:
                    output_dict[dataset_split][case_id]["label"] = [
                        os.path.join(names_dict[dataset_split]["labels"][0], label)
                    ]
...
def read_sitk_case(file_paths: List[str]):
    """
    ...
    return stacked_images.astype(np.float32), property


def read_2d_img(file_paths: List[str]):
    """
    file_paths: pathes of different modalities of the same sample
