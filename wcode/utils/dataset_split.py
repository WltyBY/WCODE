...
def dataset_split(
    ...
    if five_fold:
        ...
        for i in range(num_fold - 1):
            fold_cases_dict[str(i)] = {}

            val_cases = random.sample(list(remaining_cases), each_fold_val_length)
            train_cases = list(np.setdiff1d(case_lst, val_cases))

            fold_cases_dict[str(i)]["train"] = sorted(train_cases)
            fold_cases_dict[str(i)]["val"] = sorted(val_cases)

            remaining_cases = np.setdiff1d(remaining_cases, val_cases)
        fold_cases_dict[str(num_fold - 1)] = {}
        train_cases = list(np.setdiff1d(case_lst, remaining_cases))
        fold_cases_dict[str(num_fold - 1)]["train"] = sorted(list(train_cases))
        fold_cases_dict[str(num_fold - 1)]["val"] = sorted(list(remaining_cases))
