...
class DistillSAMProjectTrainer(PatchBasedTrainer):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.projector = nn.Sequential(
                nn.Linear(
                    in_features=self.config_dict["Network"]["features"][-1],
                    out_features=self.config_dict["Network"]["features"][-1],
                ),
                nn.ReLU(),
                nn.Linear(
                    in_features=self.config_dict["Network"]["features"][-1],
    ...
    def train_step(self, batch):
        ...
        with (
            ...
            # project
            SAM_embedding, img_embedding = (
                self.projector(
                    F.adaptive_avg_pool2d(SAM_embedding, output_size=(1, 1)).squeeze()
                ),
                self.projector(
            ...
            # print(list(self.embedding_layer.named_parameters())[0][1].data[0, 0, 0, 0])
            # print(list(self.projector.named_parameters())[0][1].data[0, 0])

            # Compute Loss
            l = self.train_loss(
                output[-len(self.pool_kernel_size) :], labels_2d
            ) + self.alpha * self.img_embeding_loss(img_embedding, SAM_embedding)
