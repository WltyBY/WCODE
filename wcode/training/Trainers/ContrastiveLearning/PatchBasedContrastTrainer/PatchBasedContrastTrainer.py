...
from wcode.training.loss.contrastive_loss import ReCo_ContrastiveLoss
...
class PatchBasedContrastTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.projector = nn.Conv3d(
                self.config_dict["Network"]["features"][0], 256, 1
            ).to(self.device)
            ...
            # self.contrastive_loss = ReCo_ContrastiveLoss()
            self.contrastive_loss = ReCo_ContrastiveLoss(
    ...
    def get_optimizers(self):
        optimizer = torch.optim.SGD(
            [*self.network.parameters(), *self.projector.parameters()],
            self.base_lr,
    ...
    def train_step(self, batch):
        ...
        with (
            ...
            proj_feature = self.projector(outputs["feature"][-1])

            # Compute Loss
            l = self.train_loss(outputs["pred"], labels) + self.contrastive_loss(
                outputs["pred"][0],
                labels[0],
                proj_feature,
            )
    ...
    def save_checkpoint(self, filename: str) -> None:
        if not self.disable_checkpointing:
            ...
            # if using deep supervision, delete unnecessary prediction_heads for lower resolutions
            if isinstance(mod.prediction_head, nn.ModuleList):
                mod.prediction_head = mod.prediction_head[-1]
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        best_saved_model = torch.load(model_path)
        self.print_to_log_file("Best Epoch:", best_saved_model["current_epoch"])
        ...
        evaluator = Evaluator(
            ...
            ground_truth_folder,
            ["DSC", "DSC_agg"],
            foreground_classes=len(self.dataset_yaml["labels"]) - 1,
