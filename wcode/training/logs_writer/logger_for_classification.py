...
class logger(object):
    ...
    def log(self, key, value, epoch: int):
        ...
        if key == "mean_accuracy":
            new_ema_pseudo_dice = (
                self.logging["ema_accuracy"][epoch - 1] * 0.9 + 0.1 * value
                if len(self.logging["ema_accuracy"]) > 0
                else value
            )
            self.log("ema_accuracy", new_ema_pseudo_dice, epoch)
    ...
    def plot_progress_png(self, output_folder):
        ...
        ax2.plot(
            x_values,
            self.logging["mean_accuracy"][: epoch + 1],
            color="g",
            ls="dotted",
            label="pseudo dice",
            linewidth=3,
        ...
        ax2.plot(
            x_values,
            self.logging["ema_accuracy"][: epoch + 1],
            color="g",
            ls="-",
