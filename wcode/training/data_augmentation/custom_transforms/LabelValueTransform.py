...
class LabelValueTransform(SegOnlyTransform):
    def __init__(
        self,
        label_value: int,
        set_to: int,
        segmentation_channels: Union[int, Tuple[int, ...], List[int]] = None,
    ):
        if (
            not isinstance(segmentation_channels, (list, tuple))
            and segmentation_channels is not None
        ):
