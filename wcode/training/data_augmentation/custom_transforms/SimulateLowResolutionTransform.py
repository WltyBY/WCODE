...
class SimulateLowResolutionTransform(ImageOnlyTransform):
    ...
    def get_parameters(self, **data_dict) -> dict:
        ...
        if self.allowed_channels is None:
            ...
            apply_to_channel = [
                i for i in self.allowed_channels if torch.rand(1) < self.p_per_channel
            ]
            ...
            if self.synchronize_axes:
                ...
                scales = torch.Tensor(
                    [
                        [
                            sample_scalar(
                                self.scale, image=data_dict["image"], channel=c, dim=d
                            )
                            for d in range(len(shape) - 1)
                        ]
                        for c in apply_to_channel
    ...
    def _apply_to_image(self, img: torch.Tensor, **params) -> torch.Tensor:
        ...
        for c, s in zip(params["apply_to_channel"], params["scales"]):
            ...
            downsampled = interpolate(
                img[c][None, None], new_shape, mode="nearest-exact"
            )
            img[c] = interpolate(
                downsampled, orig_shape, mode=self.upmodes[img.ndim - 1]
            )[0, 0]
        return img
